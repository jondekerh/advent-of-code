...
#var where the differences between the highest and lowest values of each row will be stored as a list
checksumList = []

for row in inputMatrix:
    #set variables to hold the highest and lowest value for each row
    highestValue = 0
    lowestValue = 0

    #first iteration through each int in row sets highestValue AND lowestValue to the highest int
    #this is so we will always have lowestValue as high as possible before the second iteration where we actually look for the lowest value to avoid missing anything
    for column in row:
        if column > highestValue:
            highestValue = column
            lowestValue = column

    #second iteration through each int in row sets lowestValue to the lowest int
    for column in row:
        if column < lowestValue:
            lowestValue = column

    #add the differences to checksumList
    rowDifference = highestValue - lowestValue
    checksumList.append(rowDifference)
